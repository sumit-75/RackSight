...
exports.behaviorEngine = exports.BehaviorEngine = void 0;
// In-memory simulator state
class BehaviorEngine {
    racks = [];
    ambientTemp = 21.0; // Baseline ambient temperature in Celsius
    thermalLag = 0.08; // Heat transfer/inertia coefficient (1.0 = instant, 0.0 = no change)
    thermalScale = 0.05; // Watt-to-Celsius scaling factor for target heat
    constructor() {
        ...
        for (const rack of this.racks) {
            if (rack.isStopped) {
                ...
                for (const server of rack.servers) {
                    ...
                    server.currentTemp = parseFloat((server.currentTemp + this.thermalLag * (this.ambientTemp - server.currentTemp)).toFixed(1));
                }
                continue;
            }
            for (const server of rack.servers) {
     ...
     */
    setServerStatus(rackId, serverId, status) {
        ...
        if (!server)
            throw new Error(`Server ${serverId} not found on Rack ${rackId}`);
     ...
     */
    injectFault(rackId, serverId, fault) {
        const rack = this.racks.find((r) => r.id === rackId);
        if (!rack)
            throw new Error(`Rack ${rackId} not found`);
        const server = rack.servers.find((s) => s.id === serverId);
...
exports.behaviorEngine = new BehaviorEngine();
