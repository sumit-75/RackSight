...
async function handleStop(rackIdStr: string) {
  if (!rackIdStr) {
  ...
  if (rackIdStr.toLowerCase() === 'all') {
    ...
    if (isNaN(rackId)) throw new Error('Invalid rack ID');
    await stopRack(rackId);
  }
}

async function stopRack(id: number) {
...
async function handleStart(rackIdStr: string) {
  if (!rackIdStr) {
    console.error('Error: Please specify a rack ID (1-5) or "all".');
    return;
  }
...
async function startRack(id: number) {
...
async function handleStatus(rackIdStr: string, serverIdStr: string, status: string) {
  ...
  if (isNaN(rackId) || isNaN(serverId)) throw new Error('Rack ID and Server ID must be integers');
...
async function handleFault(rackIdStr: string, serverIdStr: string, fault: string) {
